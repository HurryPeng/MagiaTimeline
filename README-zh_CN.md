...
**正式打轴**：调试模式会明显比较慢，检查对齐之后，请关闭调试模式正式打轴。恢复**配置阶段**的参数设置，运行`MagiaTimeline.bat`/`MagiaTimeline.exe`（Windows）或`MagiaTimeline.sh`（GNU/Linux/macOS）。程序运行结束后，文件夹里会出现`MagiaTimelineOutput.ass`。拿去吧你！
